...
const Signup = () => {
  ...
  // NEW STATES: User ka real data pakadne ke liye
  ...
  const handleSignup = (e) => {
    ...
    // Yahan fixed data "Admin" hata kar input wala data daal diya
    const userData = {
      name: name,   
      email: email, 
    };
    ...
    setTimeout(() => {
      login(userData); // Ab ye real data Context mein chala gaya
      navigate("/dashboard");
    }, 1500);
  };
    ...
    <div className="bg-[#f8fafc] min-h-screen flex items-center justify-center px-6 relative overflow-hidden font-sans">
      {showPopup && (
        <div className="absolute top-10 left-1/2 -translate-x-1/2 z-50 bg-slate-900 text-white px-8 py-4 rounded-2xl shadow-2xl flex items-center gap-3 animate-in slide-in-from-top-10 duration-500 border border-slate-700">
          ...
          <span className="text-sm font-bold tracking-tight">Success! Redirecting...</span>
        </div>
      )}
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-50">
          <form onSubmit={handleSignup} className="space-y-4">
            ...
            <Button type="submit" fullWidth variant="primary" className="py-4 rounded-2xl text-xs uppercase tracking-widest mt-4">
              Sign Up Now
            </Button>
          </form>
        </div>
        ...
        <p className="text-center mt-8 text-slate-400 text-xs font-bold uppercase tracking-widest">
          Already have an account? <Link to="/login" className="text-orange-500 cursor-pointer hover:underline">Login</Link>
        </p>
      </div>
    </div>
  );
